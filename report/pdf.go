...
func GeneratePDF(outputPath string, data Data) error {
	...
	m.AddRows(row.New(8).Add(
		col.New(6).Add(text.New(fmt.Sprintf("Validated Vulnerabilities: %d", countCVEs(data.Technologies)))),
		col.New(6).Add(text.New(fmt.Sprintf("Total Rounds: %d", data.Rounds), props.Text{Align: align.Right})),
	))

	if hasValidatedCVEs(data.Technologies) {
		m.AddRows(row.New(20))
		m.AddRows(row.New(15).Add(
			col.New(12).Add(
				text.New("VALIDATED CVE FINDINGS", props.Text{Size: 14, Style: fontstyle.Bold, Color: dangerColor}),
			),
		))
		...
		for _, tech := range data.Technologies {
			...
			m.AddRows(row.New(10).Add(
				col.New(12).Add(
					text.New(fmt.Sprintf("• %s (%s) - v%s [%s]", tech.Name, tech.Category, ver, tech.Confidence), props.Text{Style: fontstyle.Bold, Size: 11}),
				),
			).WithStyle(&props.Cell{BackgroundColor: &props.Color{Red: 245, Green: 245, Blue: 245}}))
			for _, e := range tech.Evidence {
				m.AddRows(row.New(7).Add(col.New(12).Add(text.New("  Evidence: "+e, props.Text{Size: 8}))))
			}
			...
			for _, cve := range tech.CVEs {
				...
				m.AddRows(row.New(7).Add(col.New(12).Add(text.New("  CVE confidence: "+cve.Confidence, props.Text{Size: 8, Style: fontstyle.Italic}))))
				for _, e := range cve.Evidence {
					m.AddRows(row.New(7).Add(col.New(12).Add(text.New("  Evidence: "+e, props.Text{Size: 8}))))
				}
			}
			m.AddRows(row.New(5))
	...
	if len(data.Technologies) > 0 {
		...
		m.AddRows(row.New(15).Add(
			col.New(12).Add(
				text.New("DETECTED TECHNOLOGIES (INFORMATIONAL)", props.Text{Size: 14, Style: fontstyle.Bold, Color: headerColor}),
			),
		))
		...
		for _, tech := range data.Technologies {
			...
			m.AddRows(row.New(9).Add(
				col.New(12).Add(
					text.New(fmt.Sprintf("• %s (%s) - v%s [%s]", tech.Name, tech.Category, ver, tech.Confidence), props.Text{Style: fontstyle.Bold, Size: 10}),
				),
			).WithStyle(&props.Cell{BackgroundColor: &props.Color{Red: 245, Green: 245, Blue: 245}}))
			for _, e := range tech.Evidence {
				m.AddRows(row.New(7).Add(col.New(12).Add(text.New("  Evidence: "+e, props.Text{Size: 8}))))
			...
			if len(tech.CVEs) == 0 {
				m.AddRows(row.New(7).Add(col.New(12).Add(text.New("  Classification: informational only; no version-specific CVE validated.", props.Text{Size: 8, Style: fontstyle.Italic}))))
			}
			m.AddRows(row.New(4))
		}
	}
	...
	if len(data.Anomalies) > 0 {
		...
		for _, a := range data.Anomalies {
			m.AddRows(row.New(10).Add(
				...
				col.New(8).Add(text.New(fmt.Sprintf("%s [%s]", a.Reason, a.Confidence), props.Text{Size: 10})),
			))
...
func hasValidatedCVEs(techs []TechInfo) bool {
	return countCVEs(techs) > 0
}

func countCVEs(techs []TechInfo) int {
	total := 0
	for _, t := range techs {
		total += len(t.CVEs)
	}
