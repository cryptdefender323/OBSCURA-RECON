...
// statusIsNoise returns true for status codes that are inherently noisy and
// should never be promoted to anomalies (e.g. generic client-error responses
// that virtually every server returns for random paths).
func statusIsNoise(status int) bool {
	switch status {
	case 400, 404, 405, 410, 429, 503:
		return true
	}
	return false
}
...
func detectAnomalies(samples []Sample, m map[string]*clusterBucket, order []string, keyOf func(Sample) string, opt Options) []Anomaly {
	...
	// Build the dominant (largest) cluster count per status code.
	...
	var out []Anomaly
	for _, s := range samples {
		// Skip inherently noisy status codes — they produce too many false positives.
		if statusIsNoise(s.Status) {
			continue
		}
		...
		// Require a meaningful dominant baseline before flagging anything.
		...
		var reason string
		switch {
		case selfN == 1 && dom >= opt.MinDominantCount:
			reason = fmt.Sprintf(
				"singleton response shape for status %d (dominant template has %d hits)",
				s.Status, dom,
			)
		case selfN*10 <= dom:
			reason = fmt.Sprintf(
				"rare response shape for status %d: cluster size %d vs dominant %d (<=10%% of dominant)",
				s.Status, selfN, dom,
			)
		default:
			continue
		...
		conf := anomalyConfidence(selfN, dom)

		// Only surface anomalies with meaningful confidence — suppress
		// "Needs manual verification" entries to avoid false-positive noise.
		if conf == "Needs manual verification" {
			continue
		}
		...
		out = append(out, Anomaly{
			...
			Confidence: conf,
			Evidence: []string{
				fmt.Sprintf("observed status=%d length=%d", s.Status, effectiveBodyLen(s)),
				fmt.Sprintf("cluster size=%d vs dominant cluster=%d for status %d", selfN, dom, s.Status),
			},
		})
	}

	// Deduplicate by (key, status, bodyLen).
...
// anomalyConfidence returns a confidence level for an anomaly.
// Only "Confirmed" and "Likely" are returned; anything weaker is suppressed
// upstream to prevent false positives.
func anomalyConfidence(selfN, dominantN int) string {
	if selfN == 1 && dominantN >= 10 {
		return "Confirmed"
	}
	if selfN*10 <= dominantN && dominantN >= 10 {
		return "Likely"
	}
